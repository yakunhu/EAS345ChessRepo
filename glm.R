...
compare_drop1_structure <- function(
    train_path = "filtered_lichess/eval_db_filtered_train.csv",
    validate_path = "filtered_lichess/eval_db_filtered_validate.csv",
  ...
  for (cand in candidates) {
    ...
    # ---- Fit model (same family/link) ----
    model <- glm(form, family = binomial(link = logit), data = dt)
    
    # ---- TRAIN predictions (core code preserved) ----
    ...
    # Train confusion + metrics (mirrors dv block below)
    ...
    # ---- VALIDATION predictions (core code preserved) ----
    ...
    # Log-likelihood helpers (core function preserved)
